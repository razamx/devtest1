...
cmake -DBUILD_TESTS=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=${dirBuildRoot}/build/host -DHOST_STRUCTURE=ON
 -DPACKAGE_TYPE=PUBLIC ${dirBuildRoot}/libraries.compute.tcc-tools
...
#cmake -DBUILD_TESTS=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=${dirBuildRoot}/build/usr -DHOST_STRUCTURE=OF
F -DPACKAGE_TYPE=PUBLIC ${dirBuildRoot}/libraries.compute.tcc-tools
# make VERBOSE=1 -j$(nproc) 2>&1 | tee <build-root>/build/build_log.txt
#make VERBOSE=1  # 2>&1 | tee ${dirBuildRoot}/build/build_log.txt
#make doc        # -j$(nproc)
#make install    # -j$(nproc)
...
#sed -i "s+path_to_detector.inf+${dirBuildRoot}/libraries.compute.tcc-tools/tools/rt_checker/efi/Detector.inf+g" ShellPk
g/ShellPkg.dsc
#build
